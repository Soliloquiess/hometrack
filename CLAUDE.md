...
**최종 갱신**: 2026-09-03 · **단계**: 개발 v1 완료 · **QA 2계층 모두 GO**(수집 `REPORT_collect` · 화면 `REPORT_site`) · **실키 미발급 → 실데이터 미검증**
...
### 다음 할 일
1. 키 발급 후: `DATA_GO_KR_KEY=… python collect.py` 1회 → `data/raw_myhome_last.json`·LH 응답 덤프로 스키마 확정 → `config.json` `_note` 미확인 상수 정리 → LH 상세 API(15057999·15056765)에 금액·면적이 있으면 SPEC §3-5 "자동 승격 규칙" 추가 + `lh_detail_confirmed: true`
2. 첫 실주행 며칠간 탭5 `new_notices` 육안 확인(복합키 폴백 공고의 제목 변경 오탐 감시) — QA A 잔존 ②
3. 잔여 조사: 럭키7하우스 2026 회차 원문 / 6인 가구 130% 원문 / 통합공공임대 맞벌이 % / 2026-06 완화 시행 여부 재확인(시행되면 `pending_change` → 값 갱신) / 민간 딥링크 실제 쿼리 문법(현재 플레이스홀더)
4. Actions 첫 실행 로그에서 키 패턴 0건 육안 확인(Q42)
